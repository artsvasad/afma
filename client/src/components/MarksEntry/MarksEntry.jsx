...
const MarksEntry = ({ selectedClass, selectedGroup, selectedVersion, students, setStudents }) => {
  ...
  const updateMark = (studentId, subjectName, field, value) => {
    ...
    setStudents(prev => prev.map(student => {
      ...
      const idx = newMarks.findIndex(m => m.subject === subjectName);
      const val = value === '' ? '' : parseInt(value) || 0;
      if (idx > -1) { newMarks[idx] = { ...newMarks[idx], [field]: val }; }
      else { newMarks.push({ subject: subjectName, term: 0, mt: 0, total: 0, [field]: val }); }
      const current = newMarks[idx > -1 ? idx : newMarks.length - 1];
      current.total = (parseInt(current.term) || 0) + (parseInt(current.mt) || 0);
  ...
  const handleAddStudent = async () => {
    ...
    setStatus('Saving...');
    try {
      const res = await api.post('/add-student', { name: newStudent.name, roll: newStudent.roll, className: selectedClass, group: selectedGroup, version: selectedVersion });
      setStudents(prev => [...prev, res.data].sort((a, b) => parseInt(a.roll) - parseInt(b.roll)));
      setNewStudent({ name: '', roll: '' }); setStatus('Saved'); rollInputRef.current?.focus();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setStatus('Saving...');
      try { await api.post('/update-marks', students); setStatus('Saved'); } catch { setStatus('Error Saving'); }
    }, 1000);
    ...
    <div className="marks-entry-container">
      <div className="add-student-bar">
        ...
        <div className="inputs">
          <input ref={rollInputRef} placeholder="Roll" value={newStudent.roll} onChange={e => setNewStudent({ ...newStudent, roll: e.target.value })} className="input-roll" />
          <input placeholder="Name" value={newStudent.name} onChange={e => setNewStudent({ ...newStudent, name: e.target.value })} onKeyDown={e => e.key === 'Enter' && handleAddStudent()} className="input-name" />
          <button onClick={handleAddStudent} className="add-btn">+ Add</button>
        </div>
      </div>
      <div className={`status-bar ${status.toLowerCase()}`}>{status === 'Idle' ? 'Ready' : status}</div>
            ...
            {students.map(student => (
              <tr key={student.id}>
                <td className="fixed-col roll-col">
                  <input className="edit-metadata-input roll-edit" value={student.roll} onChange={e => updateStudentInfo(student.id, 'roll', e.target.value)} />
                  <div className="student-id-subtext">{student.id}</div>
                </td>
                <td className="fixed-col name-col">
                  <input className="edit-metadata-input name-edit" value={student.name} onChange={e => updateStudentInfo(student.id, 'name', e.target.value)} />
                </td>
                {subjects.map(sub => {
                ...
                <td><button onClick={() => { if (window.confirm("Delete?")) api.delete(`/delete-student/${student.id}`).then(() => setStudents(p => p.filter(s => s.id !== student.id))); }} className="delete-btn"><Trash2 size={16} /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
